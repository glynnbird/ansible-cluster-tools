...
changes(DbName, Args, StartSeq) ->
    couch_log:notice("FLASH changes", []),
    changes(DbName, Args, StartSeq, []).
...
map_view(DbName, DDocInfo, ViewName, Args0) ->
    couch_log:notice("FLASH map_view", []),
    map_view(DbName, DDocInfo, ViewName, Args0, []).
...
reduce_view(DbName, DDocInfo, ViewName, Args0) ->
    couch_log:notice("FLASH reduce_view", []),
    reduce_view(DbName, DDocInfo, ViewName, Args0, []).
...
create_db(DbName) ->
    couch_log:notice("FLASH create_db", []),
...
create_shard_db_doc(_, Doc) ->
    couch_log:notice("FLASH create_shard_db_doc", []),
    rexi:reply(mem3_util:write_db_doc(Doc)).

delete_db(DbName) ->
    couch_log:notice("FLASH delete_db", []),
    couch_server:delete(DbName, []).

delete_shard_db_doc(_, DocId) ->
    couch_log:notice("FLASH delete_shard_db_doc", []),
    rexi:reply(mem3_util:delete_db_doc(DocId)).

%% @equiv get_db_info(DbName, [])
get_db_info(DbName) ->
    couch_log:notice("FLASH get_db_info", []),
    get_db_info(DbName, []).
...
open_doc(DbName, DocId, Options) ->
    couch_log:notice("FLASH open_doc", []),
    with_db(DbName, Options, {couch_db, open_doc, [DocId, Options]}).

open_revs(DbName, Id, Revs, Options) ->
    couch_log:notice("FLASH open_revs", []),
...
update_docs(DbName, Docs0, Options) ->
    couch_log:notice("FLASH update_docs", []),
